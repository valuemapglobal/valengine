...
service.interceptors.request.use(
  (config) => {
    config.meta = config.meta || {} // 开始时间记录在meta对象中
    config.meta.startTime = Date.now() // 获取开始时间
    ...
    if (config.multipleCancel) {
      // 如果设置了连续请求时取消之前的请求
      cancelControl.abort(url)
      config.cancelToken =
        cancelControl.setAbortAPI(url) /* axios  v0.22.0以下版本  */
      // config.signal = cancelControl.setAbortAPI(url)  /* axios  v0.22.0以上版本 */
    }
...
const noErrorMsgList = [
  '/vm/markdown/event/save_event',
  '/vm/event/event_user/save_event',
]
...
service.interceptors.response.use(
  (res) => {
    // 如果是 blob 类型响应，返回包含 data 和 headers 的对象，以便提取文件名
    ...
    switch (code) {
      ...
      /*case 601:
			 // 跳转到完善信息
			 window.location.href = `${window.location.origin}${window.location.pathname}#/login?type=6`
			 return*/
      case 602:
        // 跳转到联系工作人员
        Message.error('暂无权限，请联系管理员')
        window.location.href = `${window.location.origin}${window.location.pathname}#/login?type=1`
        return
      case 401:
        // 登录过期，清空token跳转到首页
