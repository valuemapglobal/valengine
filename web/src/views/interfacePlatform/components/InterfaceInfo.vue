        ...
        <div class="inner-select">
          ...
          >
            <el-form-item
              ...
              :label="item.interfaceFieldIdDescription + '：'"
              :prop="item.interfaceFieldIdAlias"
              :key="index"
            >
              <div style="display: flex">
                <el-input
                  :placeholder="$t('common.pleaseInput')"
                  v-model="interfaceForm[item.interfaceFieldIdAlias]"
                ></el-input>
              </div>
            </el-form-item>
          </el-form>
...
export default {
  ...
  watch: {
    itemInfo: {
      handler(n, o) {
        this.getfindInterfaceFieldIdInfo()
        this.getOption()
        this.res = ''
  ...
  methods: {
      ...
      this.$refs.interfaceFormRef.validate((valid) => {
        if (valid) {
          ...
          this.options.map((item, index) => {
            let key = item.interfaceFieldIdName
            if (
              [3, 4].includes(item.interfaceFieldIdDataType) &&
              interfaceForm[key]
            ) {
      ...
      findInterfaceFieldIdInfo({
        ...
        if (res.code == 200) {
          ...
          this.options = res.data.list.filter(
            (item) => item.interfaceFieldIdType === 0
          )
          this.options.forEach((item) => {
            this.$set(this.interfaceForm, item.interfaceFieldIdAlias, null)
            this.$set(this.rules, item.interfaceFieldIdAlias, [
              {
                validator: this.validateMap.get(item.interfaceFieldIdDataType),
                trigger: 'blur',
              },
            ])
          })
    ...
    // 获取参数
    getfindInterfaceFieldIdInfo() {
      findInterfaceFieldIdInfo({
        manageNo: this.itemInfo.interfaceManageNo,
      }).then((res) => {
        if (res.code == 200) {
          this.reqlist = res.data.list.filter(
            (item, index) => item.interfaceFieldIdType == 0
          )
          this.reslist = res.data.list.filter(
            (item, index) => item.interfaceFieldIdType == 1
          )
        }
      })
    },
