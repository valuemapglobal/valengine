...
//			e.printStackTrace();
    ...
    public static String idNumberDesensitization(String idNumber){
        if (idNumber.length() > 17) {
            return new StringBuffer(idNumber.trim()).replace(6,14,"********").toString();
        }
        return idNumber;
    }
    ...
    public static String userNameDesensitization(String userName){
        if (userName.length() > 1) {
            return new StringBuffer(userName.trim()).replace(1,2,"*").toString();
        }
        return userName;
    }
     ...
     * 对手机号进行脱敏
     */
    public static String phoneDesensitization(String userName){
        if (userName.length() > 1) {
            return new StringBuffer(userName.trim()).replace(3,7,"****").toString();
        }
        return userName;
    }
    public static void main(String[] args) {
        System.out.println(IDCardUtil.getAge("35082219971123531X"));
    }
}
