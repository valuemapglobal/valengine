<template>
  <div class="preApproval_classify">
    <div class="tableCard">
...
export default {
  ...
  computed: {
    ...
    getVersionControl() {
      ...
      if (obj) {
        return obj.versionObj?.newVersion
      }
    },
    mapProObj() {
  ...
  methods: {
    ...
    async reserved() {
      this.disabledVersion = true
      let modelIdList = await this.getModelIdList()
      const res = await versionReserveFraud({ ...this.getqueryData() })
      if (res.code == 200) {
        releaseStrategy({
          modelIdList,
          ...this.decision,
    ...
    delRuleGroup(data) {
      if (this.handleOperate(data)) return
      /*this.deleteText = `是否删除规则组【${data.name}】`
       this.operationInfo = { ...data, type: 1 }
       this.$refs.confirmDialog.visible = true*/
    ...
    delRule(data) {
      if (this.handleOperate(data)) return
      /*this.deleteText = `是否删除规则【${data.code}】?`
       this.operationInfo = { ...data, type: 3 }
       this.$refs.confirmDialog.visible = true;*/
      ...
      selectNewList({
        ...
        .then((res) => {
          if (res.code == 200) {
            ...
            this.tactics.total = total
            this.cardList[0].loadingMore = false
            if (!this.tactics.total) {
              this.cardList[0].noMore = false
            }
            if (
              filterParams.pageNum * filterParams.pageSize >=
                this.tactics.total &&
              this.tactics.total
            ) {
      ...
      searchCodeNewList(search)
        .then((res) => {
          if (res.code == 200) {
            ...
            if (
              this.rule.total &&
              filterParams.pageNum * filterParams.pageSize >= this.rule.total
            ) {
    ...
    onBlur(filterItem, cardIndex) {
      if (filterItem.name || filterItem.code) {
        ...
        //   ) {
        //     this.$refs.importButtonRef[cardIndex].style.background =
        //       'rgba(68, 137, 243, 0.1)'
