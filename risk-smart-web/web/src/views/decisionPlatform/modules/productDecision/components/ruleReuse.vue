<template>
  ...
  <div>
                ...
                <div class="card-item">
                  <div class="card-item-inner">
                    <div class="card-item-inner_top">
                      <el-tooltip :content="selfItem.code" placement="top">
                        <span class="title">{{ selfItem.code }}</span>
                      </el-tooltip>
                      <el-tag
            ...
            >
              ...
              >
                <div class="card-item">
                  <div class="card-item-inner">
                    <div class="card-item-inner_top">
                      <el-tooltip :content="standardItem.code" placement="top">
                        <span class="title">{{ standardItem.code }}</span>
                      </el-tooltip>
                      <el-tag
                        :type="
    ...
    >
      <AddOrEditRule :status="false" :readOnly="true" :info="formData" />
    </el-drawer>
  </div>
</template>
...
import {
  addStandardRuleRecord,
  selectPolicyGroup,
  selfBuiltRuleList,
  standardBuiltRuleGroupList,
  standardBuiltRuleList,
...
export default {
  name: 'reuse',
  components: { InfiniteLoading, AddOrEditRule },
  props: {
  ...
  data() {
    return {
      ...
      modelOptions: [], //策略下拉选择项
      groupOptions: [], //规则组下拉选择项
      //左侧——自建
      selfBuilt: {
        list: [],
        total: 0,
      ...
      standardBuilt: {
        ...
        params: {
          ...
          modelId: undefined, //策略id
          groupId: undefined, //规则组id
          pageSize: 10,
          pageNum: 1,
        },
        checkAll: false, //自建全选v-model
  ...
  methods: {
    async init() {
      ...
      if (projectCode && businessCode) this.getModelList()
    },
    getProductList() {
    ...
    getModelList() {
      this.modelOptions = []
      this.standardBuilt.params.modelId = null
      standardBuiltStrategyList({
        projectCode: this.standardBuilt.params.projectCode,
        businessCode: this.standardBuilt.params.businessCode,
        ruleCode: this.paramsData.ruleCode,
      }).then((res) => {
        if (res.code === 200) {
          this.modelOptions = res.data.list
          if (this.modelOptions.length) {
            this.standardBuilt.params.modelId = this.modelOptions[0].id
            this.getRuleGroupList()
          } else {
            this.groupOptions = []
            this.standardBuilt.params.groupId = null
            this.standardBuilt.list = []
          }
        }
      })
    },
    getRuleGroupList() {
      ...
      standardBuiltRuleGroupList({
        ...
        businessCode: this.standardBuilt.params.businessCode,
        ruleCode: this.paramsData.ruleCode,
        modelId: this.standardBuilt.params.modelId,
      }).then((res) => {
        if (res.code === 200) {
          this.groupOptions = res.data.list
          if (this.groupOptions.length) {
            this.standardBuilt.params.groupId = this.groupOptions[0].id
    ...
    getSelected() {
      selectPolicyGroup({
        ...
        moudleId: 3, //模块区分 1:策略 2:规则组 3:规则
        ruleId: this.paramsData.tacticsId, //策略id
        groupId: this.paramsData.ruleGroupId, //规则组id
      }).then((res) => {
    ...
    getList(isFirst, type) {
      const API =
        type === 'standard' ? standardBuiltRuleList : selfBuiltRuleList
      ...
      if (type === 'standard') {
        Object.assign(paramsData, {
          ...
          buildBusinessCode: this.paramsData.businessCode,
          buildRuleCode: this.paramsData.ruleCode,
          buildRuleId: this.paramsData.tacticsId,
          buildGroupId: this.paramsData.ruleGroupId,
          // ruleId: this.paramsData.tacticsId,
          modelAntiStandardRecordDataList: isFirst
            ? []
            : this.formatList(this[name].standardIncreaseList),
          modelAntiStandardRecordDataDelList: isFirst
            ? []
        ...
        Object.assign(paramsData, {
          ...
          modelId: this.paramsData.tacticsId,
          groupId: this.paramsData.ruleGroupId,
          modelAntiRecordDataList: isFirst
            ? []
            : this.formatList(this[name].selfIncreaseList),
          modelAntiRecordDataDelList: isFirst
            ? []
      ...
      API(paramsData)
        .then((res) => {
          ...
          if (type === 'standard') {
            newList = this.handleSelectedList(newList)
          }
    ...
    reuseHandle() {
      ...
      let arrs = [...arr, ...this.selfBuilt.selfIncreaseList],
        ruleRecordRuleRecordData = []
      //根据id去重
      let map = new Map()
      for (let item of arrs) {
        if (!map.has(item.id)) {
          map.set(item.id, item)
          ruleRecordRuleRecordData = [...map.values()] //把map中所有的值取出来放进数组
        }
      }
      addStandardRuleRecord({
        buildProjectCode: this.paramsData.productCode,
