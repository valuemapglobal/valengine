    ...
    <div class="reuse">
      ...
      <div class="transfer-box transfer-left">
        ...
        <div class="transfer-box-checkbox">
          ...
          >
            {{ $t('decisionPlatform.selfBuiltRuleGroupList') }}
          </el-checkbox>
          <el-checkbox-group
      ...
      <div class="transfer-box transfer-right">
        ...
        <div class="transfer-box-checkbox">
          ...
          >
            {{ $t('decisionPlatform.standardRuleGroupList') }}
          </el-checkbox>
          <el-checkbox-group
...
import {
  selfBuiltRuleGroupList,
  standardBuiltRuleGroupList,
  addStandardRuleGroup,
  selectPolicyGroup,
  ...
  methods: {
    ...
    getSelected() {
      selectPolicyGroup({
        ...
        moudleId: 2, //模块区分 1:策略 2:规则组 3:规则
    ...
    getList(isFirst, type) {
      ...
      if (type === 'standard') {
        Object.assign(paramsData, {
          ...
          modelAntiStandardGroupDataList: isFirst
            ? []
            : this.formatList(this[name].standardIncreaseList),
          modelAntiStandardGroupDataDelList: isFirst
            ? []
        ...
        Object.assign(paramsData, {
          ...
          modelAntiGroupDataList: isFirst
            ? []
            : this.formatList(this[name].selfIncreaseList),
          modelAntiGroupDataDelList: isFirst
            ? []
    ...
    reuseHandle() {
      ...
      let arrs = [...arr, ...this.selfBuilt.selfIncreaseList],
        ruleRecordRuleGroupData = []
      //根据id去重
      let map = new Map()
      for (let item of arrs) {
        if (!map.has(item.id)) {
          map.set(item.id, item)
          ruleRecordRuleGroupData = [...map.values()] //把map中所有的值取出来放进数组
        }
      }
      addStandardRuleGroup({
        buildProjectCode: this.paramsData.productCode,
        ...
        buildRuleCode: this.paramsData.ruleCode,
        ruleId: this.paramsData.tacticsId,
        ruleRecordRuleGroupData: this.formatList(ruleRecordRuleGroupData),
      }).then((res) => {
    ...
    openDetail(data) {
      ...
      const { projectCode, businessCode, modelId } = this.standardBuilt.params
